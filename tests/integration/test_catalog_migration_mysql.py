...
@pytest.fixture
def catalog_migration_database(integration_settings, monkeypatch):
    source_url = make_url(integration_settings.mysql_dsn)
    database_name = f"commercevision_catalog_migration_{uuid.uuid4().hex[:8]}"
...
def test_catalog_migration_round_trips_and_preserves_mysql_contract(
    catalog_migration_database,
) -> None:
    config, engine = catalog_migration_database
    ...
    assert {constraint["name"] for constraint in inspector.get_unique_constraints("products")} >= {
